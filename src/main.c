...
static int tool_procfs_entry_init(void)
{
    tool_procfs_entry = proc_create(TOOL_PROCFS_ENTRYNAME, TOOL_PROCFS_ENTRYPERM, NULL, &tool_procfs_fops);
    if (tool_procfs_entry == NULL)
        return 0;
    return 1;
}

static ssize_t tool_procfs_write(struct file *fp, const char __user *ubuf, size_t count, loff_t *offp)
{
    char buf[16];
    memset(buf, 0x0, 16);
...
static ssize_t tool_procfs_read(struct file *fp, char __user *buf, size_t count, loff_t *offset)
{
